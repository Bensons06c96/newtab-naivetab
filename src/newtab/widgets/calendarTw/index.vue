...
import { getTaiwanFestivalInfo, getTaiwanHolidayInfo } from './taiwan-holidays'
...
const festivalList = (() => {
  ...
  for (let i = 0; i <= 365; i++) {
    ...
    const festivalInfo = getTaiwanFestivalInfo(dateEle, lunarEle)
    if (!festivalInfo) continue
    const desc = festivalInfo.name
    if (!desc) continue
    list.push({
      date: formatDate,
      shortDate,
...
const weekList = computed(() => {
  ...
  if (localConfig.calendarTw.weekBeginsOn === 7) {
    list.unshift(sundayOption)
  } else if (localConfig.calendarTw.weekBeginsOn === 1) {
...
const genDateItem = (type: 'start' | 'main' | 'end', dateEle: typeof dayjs) => {
  ...
  // 台灣版只顯示台灣國定假日、台灣常見農曆節慶與節氣，避免混入中國大陸節慶。
  let desc = festivalInfo?.name ?? ''
  let isFestival = Boolean(desc)
  ...
  const dayType = holidayInfo?.type ?? CalendarDayType.NORMAL
...
watch(
  () => localConfig.calendarTw.weekBeginsOn,
  () => {
    onRender()
  },
)
...
const onPrevMonth = () => {
  ...
  onRender()
  gaProxy('click', ['calendarTw', 'prevMonth'])
}
...
const onNextMonth = () => {
  ...
  onRender()
  gaProxy('click', ['calendarTw', 'nextMonth'])
}
...
const onDateChange = (type: 'year' | 'month', newVal: number) => {
  ...
  gaProxy('click', ['calendarTw', 'dateChange', type])
  onRender()
}
...
const onReset = () => {
  ...
  onRender()
  gaProxy('click', ['calendarTw', 'resetToady'])
}
...
const onToggleDetailPopover = (date?: string, source?: 'clickoutside') => {
  ...
  const dateEle = dayjs(date)
  const festivalInfo = getTaiwanFestivalInfo(dateEle, lunarEle)
  const festivalDesc = festivalInfo?.name ?? ''

  detailInfo.date = `${date} 周${lunarEle.getWeekInChinese()}`
  detailInfo.lunar = `${lunarEle.getYearInGanZhi()}${lunarEle.getYearShengXiao()}年 農曆${lunarEle.getMonthInChinese()}月${lunarEle.getDayInChinese()}`
  detailInfo.solarFestivals = festivalDesc
  detailInfo.lunarFestivals = ''
  ...
  gaProxy('click', ['calendarTw', 'detail'])
}
</script>
...
<template>
  <WidgetWrap :widget-code="WIDGET_CODE">
    <div
      class="calendarTw__container calendar__container"
      :class="{
        'calendar__container--drag': isDragMode,
        'calendar__container--shadow': localConfig.calendarTw.isShadowEnabled,
        'calendar__container--border': localConfig.calendarTw.isBorderEnabled,
      }"
      :style="calendarStyle"
    >
      ...
      <NTScrollArea
        v-if="localConfig.calendarTw.festivalCountdown"
